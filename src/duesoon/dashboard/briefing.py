...
from src.duesoon.persistence.models import Assignment, NotificationDelivery, ReminderEvent, SyncRun
...
class BriefingService:
    ...
    def _items(self) -> tuple[EffectiveAssignment, ...]:
        with self.sessions() as session:
            records = session.scalars(select(Assignment).options(
                selectinload(Assignment.course), selectinload(Assignment.submission)
    ...
    def snapshot(self, now: datetime | None = None) -> dict[str, object]:
        ...
        return {
            ...
            "missing": missing, "completed_recently": completed[:10], "deadline_changes": [],
            "reminder_counts": reminder_counts,
            "freshness": {"canvas_status": "stale" if stale else "fresh", "last_synced_at": synced_at},
            "questions": [], "limitations": ["Deadline evidence is currently Canvas-only"],
        }
