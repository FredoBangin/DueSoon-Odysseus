...
The current implementation priority is a secure, single-owner web interface built directly into the DueSoon FastAPI service. It shall reuse the inherited Odysseus presentation shell directly—including its navigation hierarchy, submenus, theme system, animated backgrounds, visual primitives, and interaction behavior—without restarting the full inherited backend. DueSoon must not create a competing custom shell or visual language. Every DueSoon view, toolbar, tab, foldable section, card, form, menu, popover, modal, button, spacing rule, typography rule, animation, and responsive treatment must use the inherited Odysseus stylesheet/classes/components and be adapted from their existing patterns. Do not add new DueSoon-specific CSS for presentation. Any existing DueSoon CSS bridge is migration debt and must be removed as equivalent Odysseus markup/classes replace it. The guaranteed MVP includes owner login, the Odysseus shell, a real Canvas briefing, a Canvas-style calendar, a deterministic dashboard assistant, notification history, a Review Center foundation, and a Settings foundation. Internal synchronization freshness is diagnostic data, not decorative navigation chrome; show it only where it helps the user act.
