...
- Odysseus is the retained UI code/resource foundation, not a mandate to clone its final appearance. Reuse its proven markup, classes, components, interaction patterns, theme tokens, animations, backgrounds, and responsive behaviors where useful, while preserving the approved DueSoon dashboard/login composition. Do not force DueSoon content into an unrelated full-chat layout or create a competing UI framework; scoped DueSoon presentation adapters are allowed when the existing Odysseus primitives cannot express the approved design.
